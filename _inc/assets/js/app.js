var App = function () {
    ...
    var handleLoginForm = function () {
        ...
        $('.login-form').validate({
            ...
            invalidHandler: function (event, validator) { //display error alert on form submit   
                ...
                $('.alert-error-login', $('.login-form')).hide();
            },
            ...
            /*
             *LOGIN PELO CLICK NO SUBMIT
             */
            submitHandler: function (form) {    
                //window.location.href = "appLoginn.php";
                $.post("../_ax/axLogin.php",{ login:$('#login').val(),senha:$('#senha').val(),esqueci:"",captcha:"",rand:Math.random() } ,function(data)
                    {
                            var obj = jQuery.parseJSON(data);
                            $("#debug").html(obj.dbg);
                            if(obj.resposta=='yes') //if correct login detail
                            {
                                var ok = obj.resposta;
                                
                                var redir = 'index.php';
                                var newUrl = window.location.href.split('next=');
                                if(newUrl[1]){
                                  redir = newUrl[1];
                                }
                                document.location=redir;
                            }else{
                                //var ok = obj.resposta;alert('NAO ok1'+ok);
                                $('.alert-error', $('.login-form')).hide();
                                $('.alert-error-login', $('.login-form')).show();
                            }
                   });
            }
        });
        ...
        /*
        *LOGIN PELO BOTAO DO ENTER
        */
        $('.login-form input').keypress(function (e) {
            if (e.which == 13) {
                if ($('.login-form').validate().form()) {
                    //window.location.href = "../_ax/axLogin.php";                   
                     $.post("../_ax/axLogin.php",{ login:$('#login').val(),senha:$('#senha').val(),esqueci:"",captcha:"",rand:Math.random() } ,function(data)
                    {
                            var obj = jQuery.parseJSON(data);
                            $("#debug").html(obj.dbg);
    ...
    var handleTables = function () {
	    ...
	    // set the initial value
            ...
            "bProcessing": true					
	    ,"bServerSide": true,
	    "sAjaxSource": '../_ax/axProcurar.php?modulo='+$('#num_modulo').val()
    ...
    return {
        ...
        blockUI: function (el, loaderOnTop) {
            lastBlockedUI = el;
            jQuery(el).block({
                message: '<img src="../_inc/assets/img/loading.gif" align="absmiddle">',
                css: {
                    border: 'none',
                    padding: '2px',
