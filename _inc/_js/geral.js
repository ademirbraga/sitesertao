//Utility Functions
// $Id: geral.js,v 1.2 2009/07/20 21:30:08 janios Exp $
if (document.all)
...
function setPagina (nroPagina){
		document.getElementById('nroPagina').value = nroPagina;
		callConsulta();
	}
	
	function setOrdem (k){
		var ordem = document.getElementById('ord_'+k).value;
		ordem++;
		if(ordem>2) ordem = 0;
		document.getElementById('ord_'+k).value = ordem;
		document.getElementById('ordem').value = k;
		callConsulta();
	}
	
	function callConsulta(){
		xajax_consulta(xajax.getFormValues('formConsulta'));
	}
	
	function selecionar(campos){
		var nC = arguments.length;
		var id = arguments[arguments.length-1];
		var indx = arguments[arguments.length-2];
		var campo1 = "";
		var campo2 = "";
		var campo;
		for(var i=0; i<nC-2; i++){
			//alert(arguments[i]);
			//alert('consulta_'+arguments[i] + '['+id+']');
			
			
			campo1 = (indx>0) ? arguments[i]+'['+indx+']' : arguments[i];
			campo2 = 'consulta_'+arguments[i] + '['+id+']';
			if(i==0) campo = campo1;
			if(document.getElementById(campo1) == null){
			
				alert("Campo " + campo1 + " não existe.");
				return;
			}
			
			if(document.getElementById(campo2) == null){
				alert("Campo " + campo2 + " não existe.");
				return;
			}
			document.getElementById(campo1).value = document.getElementById(campo2).value;
		}
		closeDialog();
		if(document.getElementById(campo)!=null) document.getElementById(campo).focus();
	}

	function selecionarEditar(Modulo,ValChave){

		closeDialogEditar(Modulo,ValChave);
