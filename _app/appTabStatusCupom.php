<?php
	...
	* -------------------------------------------------------
	* Aplicativo:       appTabStatusCupom.php
	* Gerada em:        08.10.2013
	* Arquivo:          ../projSertao/_app/appTabStatusCupom.php
	...
	*/
	...
	$objPrincipal = new TabStatusCupom();
        $NomeModulo = $objPrincipal->getnomModulo();	

	if(!empty($id)){
	
		$objPrincipal->listar(NULL,$where = "cod_status = $id");
		$dados = $objPrincipal->getRegistro();	
		
		$smarty->assign('dados', $dados);
	}	

	$smarty->assign('modulo', MODULO_TABSTATUSCUPOM);
	...
	$tplConteudo = $smarty->fetch('tplTabStatusCupom.htm');
