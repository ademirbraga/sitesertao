<?php
	...
	* -------------------------------------------------------
	* Aplicativo:       appTabClassifPessoa.php
	* Gerada em:        08.10.2013
	* Arquivo:          ../projSertao/_app/appTabClassifPessoa.php
	...
	*/
	...
	$objPrincipal = new TabClassifPessoa();
        $NomeModulo = $objPrincipal->getnomModulo();	

	if(!empty($id)){
	
		$objPrincipal->listar(NULL,$where = "cod_classif_pessoa = $id");
		$dados = $objPrincipal->getRegistro();	
		
		$smarty->assign('dados', $dados);
	}	

	$smarty->assign('modulo', MODULO_TABCLASSIFPESSOA);
	...
	$tplConteudo = $smarty->fetch('tplTabClassifPessoa.htm');
