 ...
 * -------------------------------------------------------
 * Aplicativo:       appTabCliente.php
 * Gerada em:        08.08.2013
 * Arquivo:          ../sertao/_app/appTabCliente.php
 * Tabela:           tab_cliente
 ** Banco:	    bdsertao
 * -------------------------------------------------------	
 *
...
$objPrincipal = new TabCliente();
$NomeModulo = $objPrincipal->getnomModulo();

if (!empty($id)) {

    $objPrincipal->listar(NULL, $where = "cod_cliente = $id");
    $dados = $objPrincipal->getRegistro();

    $smarty->assign('dados', $dados);
}

$objTabClienteTipo = new TabClienteTipo();
$lstcod_cliente_tipo = $objTabClienteTipo->listar(NULL, $where = "cod_cliente_tipo > 1")->getPares("cod_cliente_tipo", "nom_cliente_tipo");
$smarty->assign("lstcod_cliente_tipo", $lstcod_cliente_tipo);
unset($objTabClienteTipo);

$objTabTipoPessoa = new TabTipoPessoa();
$lstcod_tipo_pessoa = $objTabTipoPessoa->formatDados("cod_tipo_pessoa", "nom_tipo_pessoa");
$smarty->assign("lstcod_tipo_pessoa", $lstcod_tipo_pessoa);
unset($objTabTipoPessoa);
...
$smarty->assign('modulo', MODULO_TABCLIENTE);
...
$tplConteudo = $smarty->fetch('tplTabCliente.htm');
?>